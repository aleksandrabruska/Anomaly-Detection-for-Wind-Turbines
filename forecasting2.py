...
base_path = r"Wind Farm A\Wind Farm A\datasets"
...
# Funkcja do określenia y_true na podstawie status_type_id i train_test
def get_y_true(df):
    if 'train_test' in df.columns and 'status_type_id' in df.columns:
        df['anomaly'] = np.where(df['train_test'] == 'prediction',
                                 np.where(df['status_type_id'].isin([0, 2]), 0, 1),
                                 0)
    else:
        raise ValueError("Brakujące kolumny 'train_test' lub 'status_type_id' w danych.")
